...
- `src/legacy.ts` — `// @ts-nocheck` carve residue. What's left: DOM event wiring, `openFile`, `commitCurrentEdit`, `rezipAndSave`, `loadFromHandle`/`pickAndOpenFolder`, locked-folder unlock, file add/rename/delete, preset overlay, scenario matrix/diff, and `hookOn(...)` registrations. **No more `_orig*` monkey-patch variables, no `patchSidebarMode` IIFE, no notes-sidecar code, no recent-scripts code, no inline context-menu helpers.** **Edit logic here only as a last resort — prefer landing changes in the relevant module.**
...
Phase 10 is complete. Build green at 189.70 kB, `tsc --noEmit` clean. Next priorities:

- **Manual smoke test.** Open `dist/index.html` in Chrome/Edge, load `M2L-KFI.OL-template`, edit a script (including applying changes and verifying the round-trip), create/clone/delete a script, try the right-click context menu (the "+ New script" picker and the right-click on a script item both went through a refactor — verify they still pop up at the right spot and dispatch the right action). Confirm scenario picker, navigator, theme panel, search, preview (data/raw/split/css modes), token strip, and Compare all work. Also flip to the Notes panel, type something, hit Ctrl+S — should write `<template-base>.notes.md` next to the template. The "Recent" strip on the scripts panel should still appear once a script has been opened.
- **GitHub Pages deploy.** Merge to `main` and verify the Pages deploy completes. Add the live URL to the README.
- **Phase 11 (optional).** What's still in `legacy.ts` (~2,475 lines): `openFile`, `commitCurrentEdit`, `rezipAndSave`, `loadFromHandle`/`pickAndOpenFolder`, locked-folder unlock, file add/rename/delete dialogs (`promptNewFile` / `openNewFileModal` / `renameFile` / `deleteFile`), preset overlay (`openPresetOverlay` + the generic `openOverlayForm`), scenario matrix/diff, scenario form, monaco "Go to script" wiring, DOM event wiring. Cleanest next targets: (a) **file-IO core** — `openFile` + `commitCurrentEdit` + `rezipAndSave` → new `src/file-ops.ts`; moderately tangled because of `afterLoadFromHandle` / `afterCommitCurrentEdit` hook registrations, but those stay in `legacy.ts`. (b) **preset overlay** — `openOverlayForm` + `closeOverlayForm` + `openPresetOverlay` + `extractPresetScalarFields` + `isPresetPath` → new `src/preset-overlay.ts`; mostly self-contained. (c) **locked-folder unlock** — `isLockedFolderMarker` + `findLockedFolderEntries` + `unlockTemplateFolders` → fold into `fs.ts` or new `src/locked-folder.ts`; small and self-contained.
