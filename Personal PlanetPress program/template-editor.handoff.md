...
- `src/legacy.ts` — `// @ts-nocheck` carve residue. What's left: DOM event wiring, `openFile`, `commitCurrentEdit`, `rezipAndSave`, `loadFromHandle`/`pickAndOpenFolder`, locked-folder unlock, file add/rename/delete, preset overlay, scenario matrix/diff, recent-scripts strip, and `hookOn(...)` registrations. **No more `_orig*` monkey-patch variables, no `patchSidebarMode` IIFE, no notes-sidecar code.** **Edit logic here only as a last resort — prefer landing changes in the relevant module.**
