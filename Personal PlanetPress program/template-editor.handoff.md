...
- `src/sidebar.ts` (new — Phase 8) — `setSidebarMode` (handles files/nav/scripts/search/theme/notes), `configureSidebar`. Replaces both the original function and the `patchSidebarMode` monkey-patch IIFE. Imports `refreshScriptsList` / `renderNavigator` / `renderThemePanel` directly.
- `src/legacy.ts` — `// @ts-nocheck` carve residue. What's left: DOM event wiring, `openFile`, `commitCurrentEdit`, `rezipAndSave`, `loadFromHandle`/`pickAndOpenFolder`, notes sidecar, locked-folder unlock, file add/rename/delete, preset overlay, scenario matrix/diff, recent-scripts strip, and `hookOn(...)` registrations. **No more `_orig*` monkey-patch variables and no more `patchSidebarMode` IIFE.** **Edit logic here only as a last resort — prefer landing changes in the relevant module.**
...
Phase 8 is complete. Build green at 191.39 kB, `tsc --noEmit` clean. Next priorities:

- **Manual smoke test.** Open `dist/index.html` in Chrome/Edge, load `M2L-KFI.OL-template`, edit a script (including applying changes and verifying the round-trip), create/clone/delete a script, try the right-click context menu. Confirm scenario picker, navigator, theme panel, search, preview (data/raw/split/css modes), token strip, and Compare all work.
- **GitHub Pages deploy.** Merge to `main` and verify the Pages deploy completes. Add the live URL to the README.
- **Phase 9 (optional).** What's still in `legacy.ts` (~2,690 lines): `openFile`, `commitCurrentEdit`, `rezipAndSave`, `loadFromHandle`/`pickAndOpenFolder`, notes sidecar, locked-folder unlock, file add/rename/delete dialogs, preset overlay, scenario matrix/diff, recent-scripts strip, DOM event wiring. The cleanest next targets are `openFile` + `commitCurrentEdit` + `rezipAndSave` → new `src/file-ops.ts`, or notes → `src/notes.ts`. Both are moderately complex because they have hook registrations woven around them.
